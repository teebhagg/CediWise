...
export default function CalculatorScreen() {
  ...
  const budgetResult =
    calcId === "budget-builder-v1"
      ? calculateBudget({ monthlyIncome: incomeNum })
      : null;
  const payeResult =
    calcId === "paye-ssnit-v1"
      ? calculatePayeSsnit({ grossSalary: grossNum })
      : null;
  const loanResult =
    ...
    calcId === "savings-goal-v1"
      ? contributionNum > 0 && targetNum > 0
        ...
        : monthsNum > 0 && targetNum > 0
          ? calculateSavingsGoal({
              targetAmount: targetNum,
              monthsToGoal: monthsNum,
            })
          : null
      : null;

  const inputStyle =
    "bg-slate-800 border border-slate-600 rounded-lg px-4 py-3 text-white text-base";
    ...
    <View style={{ flex: 1, backgroundColor: "black" }}>
      ...
      <KeyboardAvoidingView
        ...
        <Animated.ScrollView
          ...
          {calcId === "budget-builder-v1" && (
            <>
              <Text className="text-slate-300 mt-6 mb-2">
                Monthly income (GHS)
              </Text>
              <TextInput
                value={income}
                  ...
                  <View className="gap-2">
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-slate-300">
                        ₵{formatCurrency(payeResult.ssnitAmount)}
                      </Text>
                    </View>
                    <View className="flex-row justify-between">
                      <Text className="text-slate-400">PAYE</Text>
                      <Text className="text-slate-300">
                        ₵{formatCurrency(payeResult.payeAmount)}
                      </Text>
                    </View>
                    ...
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-emerald-400 font-bold">
                        ₵{formatCurrency(payeResult.netSalary)}
                      </Text>
                    </View>
                  </View>
                </View>
          ...
          {calcId === "savings-goal-v1" && (
            <>
              ...
              <TextInput
                ...
                className={inputStyle}
              />
              <Text className="text-slate-300 mt-4 mb-2">
                  ...
                  <View className="mt-6 p-4 rounded-lg bg-slate-800/80 border border-slate-600">
                    <View className="gap-2">
                      <View className="flex-row justify-between">
                        ...
                        <Text className="text-white font-medium">
                          ₵{formatCurrency(savingsResult.monthlyContribution)}
                        </Text>
                      </View>
                      <View className="flex-row justify-between">
                        <Text className="text-slate-400">Months to goal</Text>
                        <Text className="text-emerald-400 font-medium">
                          {savingsResult.monthsToGoal}
                        </Text>
                      </View>
                    </View>
                  </View>
          ...
          {calcId === "tbill-simulator-v1" && (
            <>
              ...
              <TextInput
                ...
                onChangeText={setTbillPrincipal}
                placeholder="0"
                placeholderTextColor="#64748b"
                keyboardType="numeric"
                className={inputStyle}
              />
              <Text className="text-slate-300 mt-4 mb-2">
                Annual rate (%) - override if live data stale
              </Text>
              <TextInput
                value={tbillRate}
                onChangeText={setTbillRate}
                placeholder="e.g. 25"
                placeholderTextColor="#64748b"
                keyboardType="numeric"
                className={inputStyle}
              ...
              <TextInput
                value={tbillTenor}
                onChangeText={setTbillTenor}
                placeholder="91, 182, or 364"
                placeholderTextColor="#64748b"
                keyboardType="numeric"
                className={inputStyle}
              />
                ...
                <View className="mt-6 p-4 rounded-lg bg-slate-800/80 border border-slate-600">
                  <View className="gap-2">
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-emerald-400 font-medium">
                        ₵{formatCurrency(tbillResult.interestEarned)}
                      </Text>
                    </View>
                    <View className="flex-row justify-between">
                      <Text className="text-slate-400">Maturity amount</Text>
                      <Text className="text-emerald-400 font-bold">
                        ₵{formatCurrency(tbillResult.maturityAmount)}
                      </Text>
                    </View>
                  </View>
                </View>
                ...
                <View className="mt-6 p-4 rounded-lg bg-slate-800/80 border border-slate-600">
                  <View className="gap-2">
                    <View className="flex-row justify-between">
                      ...
                      <Text className="text-emerald-400 font-medium">
                        ₵{formatCurrency(cashFlowResult.averageMonthlyNet)}
                      </Text>
                    </View>
                    <View className="flex-row justify-between">
                      <Text className="text-slate-400">
                        Total net ({cashMonthsNum} mo)
                      </Text>
                      <Text className="text-emerald-400 font-bold">
                        ₵{formatCurrency(cashFlowResult.totalNetOverPeriod)}
                      </Text>
                    </View>
                  </View>
                </View>
          ...
          <View className="mt-8 p-3 rounded-lg bg-amber-500/10 border border-amber-500/30">
            <Text className="text-amber-400 text-sm">
              ...
              tax advice. Verify with the Bank of Ghana, GRA, or a qualified
              advisor.
            </Text>
          </View>
        </Animated.ScrollView>
