...
export function useBudget(userId?: string | null): UseBudgetReturn {
  ...
  const { state, queue, isLoading, isSyncing, lastSyncRunEndedAt, retryIn } =
    store;
  ...
  const recurringSnapshot = useRecurringExpensesStore(
    (s) => s.recurringExpenses,
  );

  const budgetRecalcOptions = useCallback(
    (tc: TaxConfig | null | undefined) => ({
      recurringExpenses: useRecurringExpensesStore.getState().recurringExpenses,
      taxConfig: tc ?? undefined,
    }),
    [],
  );
  ...
  const totals = useMemo(() => {
    ...
    const { disposableIncome, totalRecurringMonthly } =
      getMonthlyDisposableIncome(
        state.incomeSources,
        recurringSnapshot,
        taxConfig ?? undefined,
      );
    const recurringByBucket = recurringTotalsByBucket(
      recurringSnapshot,
      new Date(),
    );
    ...
    const unspentThisMonth = Math.max(0, disposableIncome - spentTotal);
  ...
  }, [state, activeCycle, taxConfig, recurringSnapshot]);
  ...
  const setupBudget = useCallback(
    async ({
      ...
      const current =
        latestState ??
        useBudgetStore.getState().state ??
        createEmptyBudgetState(userId);
      const now = new Date();
      const { start, end } = computePaydayCycle(now, paydayDay);

      const startDateStr = toISODate(start);
      const endDateStr = toISODate(end);
      const existingCycle = current.cycles.find(
        (c) => c.startDate === startDateStr && c.endDate === endDateStr,
      );

      const cycleId = existingCycle ? existingCycle.id : uuidv4();
      const createdAt = existingCycle
      ...
      const cycle: BudgetCycle = {
        ...
        wantsPct,
        savingsPct,
        rolloverFromPrevious: existingCycle?.rolloverFromPrevious ?? {
          needs: 0,
          wants: 0,
          savings: 0,
        },
        reallocationApplied: existingCycle?.reallocationApplied ?? false,
        createdAt,
      ...
      for (const exp of filterEffectiveRecurringExpenses(
        recurringSnap,
        new Date(),
      )) {
        if (!exp.autoAllocate) continue;
        const monthly = toMonthlyEquivalentAmount(exp.amount, exp.frequency);
        if (monthly <= 0) continue;
        const key = exp.name.trim();
        if (!key) continue;
  ...
  const computeNewCyclePreview = useCallback((): {
    ...
    if (disposableIncome <= 0 || totalSpent >= disposableIncome) {
      return null;
    }
    ...
    const rollover = {
      needs: 0,
      wants: 0,
      savings: Math.max(0, disposableIncome - totalSpent),
    };
  ...
  }, [activeCycle, state, taxConfig, userId]);
  ...
  const createNewCycleImmediate = useCallback(async () => {
    ...
    const totalSpent = current.transactions
      ...
      .reduce((sum, t) => sum + t.amount, 0);
    const rollover =
      disposableIncome > 0 && totalSpent < disposableIncome
        ? { needs: 0, wants: 0, savings: disposableIncome - totalSpent }
  ...
  const addIncomeSource = useCallback(
    async ({
      ...
      const recalculated = recalculateBudgetFromAllocations(
        tempState,
    ...
    async (mode: BudgetEngineMode) => {
      ...
      await enqueueAndTry({
        ...
        payload: {
          ...
          ...(current.prefs.paydayDay
            ? { payday_day: current.prefs.paydayDay }
            : {}),
          ...(Array.isArray(current.prefs.interests)
            ? { interests: current.prefs.interests }
            : {}),
  ...
  const recalculateBudget = useCallback(
    async (overrideState?: BudgetState) => {
      if (!userId) return;
      const current = overrideState ?? (await loadBudgetState(userId)) ?? state;
  ...
  const syncBatch = useCallback(
    async (mutationIds: string[]) => {
      ...
      setSyncing(true);
      try {
        const result = await flushBudgetQueueUntilMutationIdsCleared(
          userId,
          mutationIds,
        );
        await refreshQueue();
