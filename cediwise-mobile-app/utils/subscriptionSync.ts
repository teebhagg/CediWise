...
export function subscriptionMatchesPayment(
  ...
  if (!row) return false;
  if (row.status === "active" && row.plan === targetTier) return true;
  if (row.status === "trial" && row.pending_tier === targetTier) return true;
  return false;
}
...
export async function waitForSubscriptionAfterPayment(
  ...
  userId: string,
  targetTier: "budget" | "sme",
  options?: { timeoutMs?: number; onTick?: () => void }
  ...
  const fetchRow = async (): Promise<SubscriptionRow | null> => {
    const { data } = await supabase
      .from("subscriptions")
      .select("plan, status, pending_tier, trial_ends_at, updated_at")
      .eq("user_id", userId)
      .maybeSingle();
    return data ?? null;
  };
